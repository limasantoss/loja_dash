...
def gerar_resposta_vendedor(pergunta, df_filtrado, df_loja_completa):
    ...
    if "faturamento" in pergunta or ("vendas" in pergunta and "dia" not in pergunta):
        ...
        start_date, end_date = df_filtrado['order_purchase_timestamp'].min().date(), df_filtrado['order_purchase_timestamp'].max().date()
        inicio_anterior, fim_anterior = get_periodo_anterior(start_date, end_date)
...
if selecao == "Visão Geral":
    ...
    if not df_filtrado.empty:
        ...
        with col_mapa:
            vendas_estado = df_filtrado['customer_state'].value_counts().reset_index()
    ...
    st.markdown(f"Analisando de **{start_date.strftime('%d/%m/%Y')}** a **{end_date.strftime('%d/%m/%Y')}**.")
    st.markdown("---")
    df_produtos = df_filtrado.dropna(subset=['product_category_name_english'])
    ...
    if not df_filtrado.empty:
        col1, col2 = st.columns(2)
        col1.metric("Tempo Médio de Entrega", f"{df_filtrado['tempo_entrega'].mean():.1f} dias")
        col2.metric("Frete Médio", f"R$ {df_filtrado['freight_value'].mean():,.2f}")
        st.markdown("---")
        st.subheader("Performance Logística por Estado")
        col_graf1, col_graf2 = st.columns(2)
        with col_graf1:
            tempo_estado = df_filtrado.groupby('customer_state')['tempo_entrega'].mean().sort_values(ascending=False).reset_index()
            ...
            st.plotly_chart(fig, use_container_width=True)
        with col_graf2:
            frete_estado = df_filtrado.groupby('customer_state')['freight_value'].mean().sort_values(ascending=False).reset_index()
        ...
        with col_hist:
            fig_hist = px.histogram(df_filtrado, x='tempo_entrega', nbins=30, title="Distribuição do Tempo de Entrega")
            st.plotly_chart(fig_hist, use_container_width=True)
        with col_scatter:
            df_scatter = df_filtrado.groupby('customer_state').agg(tempo_medio=('tempo_entrega', 'mean'), frete_medio=('freight_value', 'mean'), pedidos=('order_id', 'count')).reset_index()
